...
function LevelBadge({ level }: { level: WorkshopLevel }) {
  return (
    <span className={`inline-block px-2.5 py-0.5 rounded-full text-[11px] font-medium tracking-wide ${levelColors[level]}`}>
      {levelLabels[level]}
    </span>
  )
}
        ...
        <div className="absolute inset-0 bg-espresso/20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <div className="w-12 h-12 rounded-full bg-parchment/90 flex items-center justify-center">
            <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
              <rect x="4" y="9" width="12" height="9" rx="2" stroke="#2c241b" strokeWidth="1.4" />
              <path d="M7 9V6.5a3 3 0 0 1 6 0V9" stroke="#2c241b" strokeWidth="1.4" strokeLinecap="round" />
    ...
    <div className="min-h-screen bg-parchment text-espresso">
      ...
      <section className="relative h-[420px] md:h-[500px] overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col justify-end pb-14 md:pb-20 px-6 md:px-12 lg:px-20 max-w-5xl">
          <p className="label-caps text-linen/70 mb-4 tracking-widest">Видео-курсы</p>
          ...
          <p className="text-parchment/80 max-w-lg leading-relaxed text-lg">
            Пошаговые видео-уроки от мастера Stariva — от первого узла до готового изделия.
          </p>
        </div>
      </section>
      ...
      <div className="bg-sand border-y border-espresso/8">
        <div className="max-w-6xl mx-auto px-6 md:px-10 py-5 flex flex-wrap items-center justify-center gap-8 md:gap-14">
          {[
            { icon: "M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z", label: "Доступ навсегда" },
            { icon: "M15 10l4.553-2.069A1 1 0 0121 8.82v6.36a1 1 0 01-1.447.894L15 14M3 8a2 2 0 012-2h10a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2V8z", label: "Видео в HD" },
            { icon: "M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z", label: "С любого устройства" },
            { icon: "M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0", label: "Поддержка мастера" },
          ].map((item) => (
            <div key={item.label} className="flex items-center gap-2 text-espresso/70">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" className="text-terracotta flex-shrink-0">
                <path d={item.icon} stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
              <span className="label-caps-md text-espresso/80">{item.label}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <main className="max-w-6xl mx-auto px-6 md:px-10 py-16 md:py-20">
        ...
        {featured.length > 0 && (
          <section className="mb-16">
            <div className="flex items-baseline gap-4 mb-8">
              <h2 className="font-serif text-3xl text-espresso">Популярные курсы</h2>
        ...
        {/* Category sections */}
        {(["lampshades", "clothing", "decor", "interior"] as WorkshopCategory[]).map((cat) => {
          const catWorkshops = workshops.filter((w) => w.category === cat)
          if (catWorkshops.length === 0) return null
          return (
            <section key={cat} className="mb-14">
              <div className="flex items-baseline gap-4 mb-8">
                <h2 className="font-serif text-2xl text-espresso">{categoryLabels[cat]}</h2>
                <div className="h-px flex-1 bg-espresso/10" />
                <span className="text-sm text-taupe">{catWorkshops.length} курс{catWorkshops.length > 1 ? "а" : ""}</span>
              </div>
